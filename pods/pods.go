...
func GetPodState(root string, uuid string) (PodState, error) {
    ...
    if err != nil && !os.IsNotExist(err) { return Invalid, err }
    if err == nil { return Embryo, nil }
    ...
    if err != nil && !os.IsNotExist(err) { return Invalid, err }
    if err == nil { return Prepared, nil }
    ...
    if err != nil && !os.IsNotExist(err) { return Invalid, err }
    if err == nil { return ExitedGarbage, nil }

    lockable, err = canLock(getPodDir(root, uuid, "Garbage"))
    if err != nil && !os.IsNotExist(err) { return Invalid, err }
    if err == nil { return Garbage, nil }
